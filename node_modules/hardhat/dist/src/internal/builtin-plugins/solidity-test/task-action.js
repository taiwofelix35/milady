...
const runSolidityTests = async ({ testFiles, chainType, grep, noCompile, testSummaryIndex }, hre) => {
    ...
    const runStream = run(chainType, edrArtifactsWithMetadata.map(({ edrArtifact }) => edrArtifact), testSuiteIds, testRunnerConfig, tracingConfig, sourceNameToUserSourceName, runOptions);
    let failed = 0;
    let passed = 0;
    let skipped = 0;
    let failureOutput = "";
